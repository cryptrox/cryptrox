 ...
 * between Cryptroxcoind, and Cryptroxcoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-algo=<algo>", strprintf("Mining algorithm: x16r (default: x16r)"), false, OptionsCategory::BLOCK_CREATION);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/cryptrox/cryptrox>";
    const std::string URL_WEBSITE = "<https://cryptrox.org>";
...
/** Sanity checks
 *  Ensure that Cryptroxcoin is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    if (strAlgo == "x16r")
         miningAlgo = ALGO_X16R; // CRYPTROX TODO: we should not be here
...
{
    // Make sure only a single Cryptroxcoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Cryptroxcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Cryptroxcoin is started while in a temporary directory.\n",
