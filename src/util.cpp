...
const char * const BITCOIN_CONF_FILENAME = "cryptrox.conf";
const char * const BITCOIN_PID_FILENAME = "cryptroxd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "cryptroxd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Cryptrox
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Cryptrox
    // Mac: ~/Library/Application Support/Cryptrox
    // Unix: ~/.cryptrox
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cryptrox";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cryptrox";
#else
    // Unix
    return pathRet / ".cryptrox";
#endif
#endif
}
...
const fs::path &GetBackupsDir()
{
    ...
    if (gArgs.IsArgSet("-walletbackupsdir")) {
        ...
        // CRYPTROX TODO: SetMiscWarning(strMessage);
    }
    // Default path
