...
const std::string strMessageMagic = "Cryptrox Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, CBlockHeader pblock, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    // CRYPTROX BEGIN
    if (nHeight == 1)
        return 25000000 * COIN;        // Premine 25 million coins
    else if (nHeight >= 2 || nHeight <= 2500000000)
        return 0.05 * COIN;               // Default subsidy 0.05 coins
    else
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if(nHeight >= nMNPIBlock) ret = 1.25 * COIN; // 1.25 coins
...
void ThreadScriptCheck() {
    RenameThread("cryptrox-scriptch");
