...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params) {
    ...
    // make sure we have at least 5 blocks, otherwise just return powLimit
    ...
    // CRYPTROX instamine protection for blockchain
    ...
    {
        // CRYPTROX instamine protection for algo
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Dead lock protection will halve work every block spacing when no block for 2 * number of active algos * block spacing (Cryptrox: every 2.5 minutes if no block for 5 minutes)
...
unsigned int GetHandbrakeForce(int32_t nVersion, int nHeight)
{
    ...
    if (nHeight >= sporkManager.GetSporkValue(SPORK_CRYPTROX_01_HANDBRAKE_HEIGHT))
    {
        switch (nVersionAlgo)
        {
            case ALGO_X16R:    return sporkManager.GetSporkValue(SPORK_CRYPTROX_01_HANDBRAKE_FORCE_X16R);
